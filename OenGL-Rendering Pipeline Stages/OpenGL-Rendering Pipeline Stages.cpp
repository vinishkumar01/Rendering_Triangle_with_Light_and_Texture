#define STB_IMAGE_IMPLEMENTATION // we are defining this because this helps setting up the STB to treat the header file as more of a code file as opposed to header
...
//       x     y    z       u     v			u and v are the texture coordinates
		...
		0.0f, -1.0f, 1.0f,	0.5f, 0.0f,			// background for indepth so that the object could be created as pyramid shape.............1
		1.0f,-1.0f,0.0f,	1.0f, 0.0f,			//  Bottom Right..............2
		0.0f,1.0f,0.0f,		0.5f, 1.0f			//  Top...............3
	};

	Mesh* obj1 = new Mesh();
	obj1->CreateMesh(triangle_vertices, triangle3D_indices, 20, 12);
	meshList.push_back(obj1);

	Mesh* obj2 = new Mesh();
	obj2->CreateMesh(triangle_vertices, triangle3D_indices, 20, 12);
...
int main() {
	...
	CosmosTex = Texture((char*)("D:/GL-Projects/OpenGL-Rendering Pipeline Stages_and_advanced/OenGL-Rendering Pipeline Stages/Textures/wall.png"));
	CosmosTex.loadTexture();
	PatternTex = Texture((char*)("D:/GL-Projects/OpenGL-Rendering Pipeline Stages_and_advanced/OenGL-Rendering Pipeline Stages/Textures/paper.png"));
	PatternTex.loadTexture();
	...
	while (!MainScreen.getShouldClose())
	{
		...
		camera.KeyControls(MainScreen.getKeys(), deltaTime);
		camera.MouseControls(MainScreen.GetXChange(), MainScreen.GetYChange());
